...
# Задание 3 -  Environment Variables
...
## 2. Запускаем контейнер first_node из образа node версии 15.14 в фоновом режиме, подключив папку data из текущей директории в /var/first/data контейнера:
```sh
docker run -d --name first_node -v d:\/projects/education/education/data:/var/first/data node:15.14 sleep infinity
04ef3b64b5e001a0967c6c0900241ca49dc55479eff78aa5679000ad48fadde6
```

## 3. Запускаем контейнер second_node из образа node версии 15.14 в фоновом режиме, подключив папку data из текущей директории в /var/second/data контейнера:
```sh
docker run -d --name second_node -v d:\/projects/education/education/data:/var/second/data node:15.14 sleep infinity
dd9f5ffbbd1814fc257224fea8d2bf831455b5428c7bba162ce8520d8487b03b
```

## 4. Подключаемся к контейнеру first_node с помощью exec и создаём текстовый файл:
```sh
docker exec -it first_node bash

root@04ef3b64b5e0:/# cd var/first/data
root@04ef3b64b5e0:/var/first/data# touch first-node.txt && echo "Test text" > first-node.txt
root@04ef3b64b5e0:/var/first/data# cat first-node.txt 
Test text
root@04ef3b64b5e0:/var/first/data#
```

## 5. Добавляем еще один файл в папку data на хостовой машине:
```sh
создаём новый файл и проверяем:
root@04ef3b64b5e0:/var/first/data# ls
first-node.txt  new.txt
root@04ef3b64b5e0:/var/first/data# exit
exit
```

## 6. Подключаемся к контейнеру second_node с помощью exec и получаем список файлов в директории /var/second/data, выводим на экран содержимое файлов:
```sh
docker exec -it second_node bash

root@dd9f5ffbbd18:/# cd /var/second/data
root@dd9f5ffbbd18:/var/second/data# ls
first-node.txt  new.txt
root@dd9f5ffbbd18:/var/second/data#
root@dd9f5ffbbd18:/var/second/data# cat first-node.txt 
Test text
root@dd9f5ffbbd18:/var/second/data# cat new.txt 
new file!root@dd9f5ffbbd18:/var/second/data# 
```

## 7. Останавливаем оба контейнера:
```sh
root@dd9f5ffbbd18:/var/second/data# exit
exit
...
## 9. Удаляем образ node версии 15.14:
```sh
$ docker rmi node:15.14
Untagged: node:15.14
Untagged: node@sha256:608bba799613b1ebf754034ae008849ba51e88b23271412427b76d60ae0d0627
